...
double PIDController::Control(const double error, const double dt) {
    if (dt <= 0) {
        // SG_ERROR("dt <= 0, will use the last output, dt: %f", dt);
    ...
    if (!integrator_enabled_) {
        ...
        // state
        // SG_INFO("intergral = %lf,error = %lf,dt = %lf,ki = %lf", integral_,
        //         error, dt, ki_);
...
void PIDController::Init(const PidConf &pid_conf) {
    ...
    integrator_saturation_high_ = pid_conf.integrator_saturation_high;
    integrator_saturation_low_ = pid_conf.integrator_saturation_low;
    integrator_saturation_status_ = 0;
    integrator_hold_ = false;
    output_saturation_high_ = pid_conf.output_saturation_level;
    output_saturation_low_ = -pid_conf.output_saturation_level;
    output_saturation_status_ = 0;
    SetPID(pid_conf);
}
